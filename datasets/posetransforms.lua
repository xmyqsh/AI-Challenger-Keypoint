...
function M.getTransform(center, scale, rot, res)
    ...
    -- Translation
    t[1][3] = res * (-center[1] / h[1] + .5)
    t[2][3] = res * (-center[2] / h[2] + .5)
...
function M.crop(img, center, scale, rot, res)
    ...
    if newDim:size(1) > 2 then
        ...
        --[[
        if not pcall(function() newImg:sub(1,newDim[1],newY[1],newY[2],newX[1],newX[2]):copy(img:sub(1,newDim[1],oldY[1],oldY[2],oldX[1],oldX[2])) end) then
          ...
          print(scale)
          print("new_")
          print(newY[1],newY[2],newX[1],newX[2])
          print("old_")
          print(oldY[1],oldY[2],oldX[1],oldX[2])
        end
        --]]
    else
        newImg:sub(newY[1],newY[2],newX[1],newX[2]):copy(img:sub(oldY[1],oldY[2],oldX[1],oldX[2]))
        --[[
        if not pcall(function() newImg:sub(newY[1],newY[2],newX[1],newX[2]):copy(img:sub(oldY[1],oldY[2],oldX[1],oldX[2])) end) then
          print("Error occurred during crop!")
          print("center")
          print(center)
          print("scale")
...
function M.crop2(img, center, scale, rot, res)
    ...
    local c,s = center, scale
    print(center)
    print(scale)
    print(scaleFactor)
    c[1] = center[1]:float() / scaleFactor[1]
    c[2] = center[2]:float() / scaleFactor[2]
    s[1] = scale[1] / scaleFactor[1]
    s[2] = scale[2] / scaleFactor[2]
    local ul = M.transform({1,1}, c, s, 0, res, true)
    local br = M.transform({res+1,res+1}, c, s, 0, res, true)
    if scaleFactor:max() >= 2 then br:add(-(br - ul - res)) end
    ...
    if scaleFactor:max() < 2 then newImg = image.scale(newImg,res,res) end
    if ndim == 2 then newImg = newImg:view(newImg:size(2),newImg:size(3)) end
    return newImg
end
...
-- Draw gaussian
-------------------------------------------------------------------------------
function M.drawGaussian(img, pt, tmpSize)
    -- Draw a 2D gaussian
    -- Check that any part of the gaussian is in-bounds
    -- local tmpSize = math.ceil(3*sigma)
    local ul = {math.floor(pt[1] - tmpSize[1]), math.floor(pt[2] - tmpSize[2])}
    local br = {math.floor(pt[1] + tmpSize[1]), math.floor(pt[2] + tmpSize[2])}
    ...
    local g = image.gaussian(7 , 0.25, 1, True, size[1], size[2])
    --[[
    print("size")
    print(size)
    print("g")
