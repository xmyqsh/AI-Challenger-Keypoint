...
function Trainer:train(epoch, dataloader)
  ...
  local predFilePath = paths.concat(self.opt.save, self.opt.expID, 'train_pred_post_' .. epoch .. '.h5')
  local imgsetFilePath = paths.concat('data', self.opt.dataset, 'train_images.txt')
  local predJsonFilePath = paths.concat('data', self.opt.dataset, 'keypoint_predictions.json')
  local annotFilePath= paths.concat('data', self.opt.dataset, 'annotations', 'train.json')

  local predFile = hdf5.open(predFilePath, 'w')
  predFile:write('preds', preds)
  predFile:close()

  -- os.execute('sh prediction_json_gen.sh checkpoints/AI/AI_hg8/pred_post_6.h5 data/AI/valid_images.txt data/AI/keypoint_predictions.json')
  local sth = 'sh prediction_json_gen.sh ' .. predFilePath .. ' ' .. imgsetFilePath .. ' ' .. predJsonFilePath
  os.execute('sh utils/prediction_json_gen.sh ' .. predFilePath .. ' ' .. imgsetFilePath .. ' ' .. predJsonFilePath)
  os.execute('sh utils/keypoint_eval.sh ' .. predJsonFilePath .. ' ' .. annotFilePath .. ' 1')
  mAP = sys.execute('sh utils/keypoint_eval.sh ' .. predJsonFilePath .. ' ' .. annotFilePath .. ' 0')

   return mAP, accSum / N, lossSum / N
end
...
function Trainer:valid(epoch, dataloader)
   ...
   self.model:training()
    ...
    --[[
    local predFilePath = paths.concat(self.opt.save, self.opt.expID, 'validation_pred_post_' .. epoch .. '.h5')
    local imgsetFilePath = paths.concat('data', self.opt.dataset, 'validation_images.txt')
    --]]
    local predFilePath = paths.concat(self.opt.save, self.opt.expID, 'det_validation_pred_post_' .. epoch .. '.h5')
    local imgsetFilePath = paths.concat('data', self.opt.dataset, 'det_validation_images.txt')
    local predJsonFilePath = paths.concat('data', self.opt.dataset, 'keypoint_predictions.json')
    local annotFilePath= paths.concat('data', self.opt.dataset, 'annotations', 'validation.json')
...
function Trainer:test(epoch, dataloader)
   ...
   self.model:training()
    ...
    local predFilePath = paths.concat(self.opt.save, self.opt.expID, 'test_pred_post_' .. epoch .. '.h5')
    local predFile = hdf5.open(predFilePath, 'w')
    predFile:write('preds', preds)
    predFile:close()
