         ...
         threads:addjob(
            function(indices)
               ...
               for i, idx in ipairs(indices:totable()) do
                  ...
                  if torch.type(sample.target) == 'table' then
                    for n = 1, nStack do table.insert(targetTable, nil) end
                    for n = 1, nStack do
                      if not targetTable[n] then 
                         targetTable[n] = sample.target[n]:view(1,unpack(sample.target[n]:size():totable())) 
                      else 
                         targetTable[n] = targetTable[n]:cat(sample.target[n]:view(1,unpack(sample.target[n]:size():totable())),1) 
                      end
                    end
                  else
                    if not target then 
